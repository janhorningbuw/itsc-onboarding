const taskTemplate = [
  ...
  ['key','week','Complete key request form','Send the signed form to the dean via Hauspost.',2], ['stationery','week','Get stationary','Collect office supplies in room FME 01.06.',3],
  ['equipment','week','Set up equipment','Order anything missing together with the office coordinator.',4], ['ethernet','week','Set up ethernet and Wi-Fi','Use the assigned IP settings and verify access via the ZIM account.',5],
...
const taskResources = {
  ...
  photo: `<details class="task-extra"><summary>Show photo submission address</summary><p>Send your photo by email to <a href="mailto:gellert@uni-wuppertal.de">gellert@uni-wuppertal.de</a>.</p></details>`,
  ...
  campus: `<details class="task-extra"><summary>Show campus information</summary><p>Use the Campus Freudenberg map to locate the office, parking areas and important facilities.</p></details>`,
...
function render() {
  ...
  renderProgress(); renderChecklists(); renderCalendar(); renderProfile();
}
...
function renderChecklists() {
  ...
  $('#checklists').innerHTML = preparation + sections.map(([id,eyebrow,title]) => {
    const tasks = data.tasks.filter(task => task.section === id); const requiredTasks = tasks.filter(task=>!isOptionalTask(task)); const done = requiredTasks.filter(task => task.done).length;
